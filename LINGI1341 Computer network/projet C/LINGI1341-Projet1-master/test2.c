...
void read_write_loop(int sfd){
	...
	struct timeval *timer_2 = malloc(sizeof(struct timeval));
	timer->tv_sec=5;
	timer->tv_usec=0;
	...
	fd_set *lecture_alt = malloc(sizeof(fd_set));
	FD_ZERO(lecture_alt);
	FD_SET(sfd, lecture_alt);

	fd_set *ecriture_alt = malloc(sizeof(fd_set));
	FD_ZERO(ecriture_alt);
	FD_SET(sfd, ecriture_alt);
	...
	while(boolean){
		...
		if(try<0){
			...
			if(FD_ISSET(sfd, lecture)){
				while(recv(sfd, (void *) read_buffer,1024,MSG_DONTWAIT)>0){
				//write(STDOUT_FILENO, (void *) read_buffer, 1);
				//printf("YOLO\n");
				//nbr_elem_rbuf= nbr_elem_rbuf+1;
				//memset((void *) read_buffer, '\0', nbr_elem_rbuf);
				//copy_try = select(sfd+1,lecture_alt,NULL,NULL,timer_2);
				}
				//read_buffer[nbr_elem_rbuf]='\0';
				printf("RECU: %s\n", read_buffer);
				//nbr_elem_rbuf = 0;
				//nbr_elem_rbuf=0;
				try=-1;
			...
			if(FD_ISSET(sfd, ecriture) && datain_w){
				char to_send = write_buffer[0];
				if(to_send=='\0'){
					boolean = FALSE;
				} else {
					int copy_ty = try;
					while(copy_ty>0){
					write(sfd, (void *) write_buffer, 1);
					memcpy((void *) write_buffer, (void *) &write_buffer[1], nbr_elem_wbuf-1);
					memset((void *) &write_buffer[nbr_elem_wbuf-1], '\0', 1);
					nbr_elem_wbuf--;
					FD_ZERO(ecriture_alt);
					FD_SET(sfd, ecriture_alt);
					copy_ty = select(sfd+1, NULL,ecriture_alt, NULL, timer_2);	
					if(write_buffer[0]=='\0'){
						datain_w=FALSE;
						copy_ty=-1;
					}
			...
			if(FD_ISSET(STDIN_FILENO, lecture)){
				char lecture2[1024];
				memset(lecture2, '\0', 1024);
				read(STDIN_FILENO, (void *) lecture2, 1024);
				memcpy((void *) &write_buffer[nbr_elem_wbuf], (void *) lecture2, (int) strlen(lecture2));
				nbr_elem_wbuf=nbr_elem_wbuf+(int) strlen(lecture2);				
				datain_w=TRUE;
				try=-1;
			}
		}
