	...
	while(boolean){
		...
		if(try<0){
			...
			if(FD_ISSET(sfd, lecture)){
				int ret = read(sfd, (void *) read_buffer,1024);
                		if(ret==0){
                   		  return;
               			 }
			...
			//		boolean = FALSE;
			//	}
			//	write(sfd, (void *) write_buffer, strlen(write_buffer));
			//	memset((void *) write_buffer, '\0', strlen(write_buffer));
			//	datain_w=FALSE;
			//}
			if(FD_ISSET(STDIN_FILENO, lecture)){
				//memset((void *) temp_buf, '\0', 1024);
				int ret2 = read(STDIN_FILENO, (void *) write_buffer, 1024);
